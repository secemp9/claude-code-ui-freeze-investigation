...
var otA = v(ME8 => {
  ...
  Object.defineProperty(ME8, "__esModule", {
    value: !0
  });
  var QX = __$.sq(),
    O6A = __$.H8(),
  ...
  class JV {
    ...
    constructor(A = {}) {
      JV.prototype.__init.call(this), JV.prototype.__init2.call(this), JV.prototype.__init3.call(this), JV.prototype.__init4.call(this), JV.prototype.__init5.call(this), JV.prototype.__init6.call(this), this._options = {
    ...
    setupOnce(A) {
      ...
      K.subscribe(ME8.ChannelName.RequestCreate, this._onRequestCreate), K.subscribe(ME8.ChannelName.RequestEnd, this._onRequestEnd), K.subscribe(ME8.ChannelName.RequestError, this._onRequestError);
    }
    _shouldCreateSpan(A) {
      if (this._options.tracing === !1 || this._options.tracing === void 0 && !QX.hasTracingEnabled()) return !1;
    ...
    __init4() {
      this._onRequestCreate = A => {
        if (!rtA([QX.getClient, "call", $ => $(), "optionalAccess", $ => $.getIntegration, "call", $ => $(JV)])) return;
        let {
            request: K
          } = A,
        ...
        if (($ => {
          ...
          let {
            ...
            W = O ? QX.spanToTraceHeader(O) : O6A.generateSentryTraceHeader($, _, G),
            D = O6A.dynamicSamplingContextToSentryBaggageHeader(Z || (O ? QX.getDynamicSamplingContextFromSpan(O) : QX.getDynamicSamplingContextFromClient($, Y, w)));
          beq(K, W, D);
        }
      };
    }
    __init5() {
      this._onRequestEnd = A => {
        if (!rtA([QX.getClient, "call", w => w(), "optionalAccess", w => w.getIntegration, "call", w => w(JV)])) return;
        let {
            request: K,
            response: q
          } = A,
        ...
        if (this._options.breadcrumbs) QX.addBreadcrumb({
          category: "http",
          data: {
            method: K.method,
    ...
    __init6() {
      this._onRequestError = A => {
        if (!rtA([QX.getClient, "call", z => z(), "optionalAccess", z => z.getIntegration, "call", z => z(JV)])) return;
        let {
            request: K
          } = A,
          q = K.origin ? K.origin.toString() + K.path : K.path;
  ...
  function beq(A, K, q) {
    let Y;
    if (Array.isArray(A.headers)) Y = A.headers.some(z => z === "sentry-trace");else Y = A.headers.split(`\r
`).some(w => w.startsWith("sentry-trace:"));
    if (Y) return;
    if (A.addHeader("sentry-trace", K), q) A.addHeader("baggage", q);
  }
  function xeq(A, K, q) {
    let Y = O6A.parseUrl(q),
      z = K.method || "GET",
      w = {
        "http.method": z
      };
    ...
    return rtA([A, "optionalAccess", H => H.startChild, "call", H => H({
      op: "http.client",
      origin: "auto.http.node.undici",
      description: `${z} ${O6A.getSanitizedUrlString(Y)}`,
      data: w
    })]);
  }
  ME8.Undici = JV;
  ME8.nativeNodeFetchintegration = heq;
});
