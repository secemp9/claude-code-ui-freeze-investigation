// Module: R$A
// Dependencies: ZSA, vY1, WSA, EY1, Gb4
...
var R$A = v((Djw, jb4) => {
  var kY1 = __$.ZSA(),
    ...
    ov9 = __$.EY1(),
    {
      compareIdentifiers: L$A
    } = __$.Gb4();
  class Uy {
    constructor(A, K) {
      if (K = ov9(K), A instanceof Uy) {
      ...
      if (A.length > Zb4) throw TypeError(`version is longer than ${Zb4} characters`);
      kY1("SemVer", A, K), this.options = K, this.loose = !!K.loose, this.includePrerelease = !!K.includePrerelease;
      let q = A.trim().match(K.loose ? Wb4[LY1.LOOSE] : Wb4[LY1.FULL]);
      if (!q) throw TypeError(`Invalid Version: ${A}`);
      if (this.raw = A, this.major = +q[1], this.minor = +q[2], this.patch = +q[3], this.major > CY1 || this.major < 0) throw TypeError("Invalid major version");
      if (this.minor > CY1 || this.minor < 0) throw TypeError("Invalid minor version");
      if (this.patch > CY1 || this.patch < 0) throw TypeError("Invalid patch version");
      if (!q[4]) this.prerelease = [];else this.prerelease = q[4].split(".").map(Y => {
        if (/^[0-9]+$/.test(Y)) {
          let z = +Y;
          if (z >= 0 && z < CY1) return z;
        }
        return Y;
      });
    ...
    compare(A) {
      if (kY1("SemVer.compare", this.version, this.options, A), !(A instanceof Uy)) {
        if (typeof A === "string" && A === this.version) return 0;
        A = new Uy(A, this.options);
      }
      if (A.version === this.version) return 0;
      return this.compareMain(A) || this.comparePre(A);
    }
    compareMain(A) {
      if (!(A instanceof Uy)) A = new Uy(A, this.options);
      return L$A(this.major, A.major) || L$A(this.minor, A.minor) || L$A(this.patch, A.patch);
    }
    comparePre(A) {
      if (!(A instanceof Uy)) A = new Uy(A, this.options);
      ...
      do {
        let q = this.prerelease[K],
          Y = A.prerelease[K];
        if (kY1("prerelease compare", K, q, Y), q === void 0 && Y === void 0) return 0;else if (Y === void 0) return 1;else if (q === void 0) return -1;else if (q === Y) continue;else return L$A(q, Y);
      } while (++K);
    }
    compareBuild(A) {
      if (!(A instanceof Uy)) A = new Uy(A, this.options);
      ...
      do {
        let q = this.build[K],
          Y = A.build[K];
        if (kY1("build compare", K, q, Y), q === void 0 && Y === void 0) return 0;else if (Y === void 0) return 1;else if (q === void 0) return -1;else if (q === Y) continue;else return L$A(q, Y);
      } while (++K);
    }
    inc(A, K, q) {
      if (A.startsWith("pre")) {
        ...
        if (K) {
          let Y = new RegExp(`^${this.options.loose ? Db4[LY1.PRERELEASELOOSE] : Db4[LY1.PRERELEASE]}$`),
            z = `-${K}`.match(Y);
