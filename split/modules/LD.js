// Module: LD
// Dependencies: cyA, dyA, hXA, l51, aK6
...
var LD = v((OYw, nX4) => {
  var i51 = __$.cyA(),
    ...
    je3 = __$.l51(),
    {
      compareIdentifiers: bXA
    } = __$.aK6();
  class Sy {
    constructor(A, K) {
      if (K = je3(K), A instanceof Sy) {
      ...
      if (A.length > iX4) throw TypeError(`version is longer than ${iX4} characters`);
      i51("SemVer", A, K), this.options = K, this.loose = !!K.loose, this.includePrerelease = !!K.includePrerelease;
      let q = A.trim().match(K.loose ? r51[o51.LOOSE] : r51[o51.FULL]);
      if (!q) throw TypeError(`Invalid Version: ${A}`);
      if (this.raw = A, this.major = +q[1], this.minor = +q[2], this.patch = +q[3], this.major > n51 || this.major < 0) throw TypeError("Invalid major version");
      if (this.minor > n51 || this.minor < 0) throw TypeError("Invalid minor version");
      if (this.patch > n51 || this.patch < 0) throw TypeError("Invalid patch version");
      if (!q[4]) this.prerelease = [];else this.prerelease = q[4].split(".").map(Y => {
        if (/^[0-9]+$/.test(Y)) {
          let z = +Y;
          if (z >= 0 && z < n51) return z;
        }
        return Y;
      });
    ...
    compare(A) {
      if (i51("SemVer.compare", this.version, this.options, A), !(A instanceof Sy)) {
        if (typeof A === "string" && A === this.version) return 0;
        A = new Sy(A, this.options);
      }
      if (A.version === this.version) return 0;
      return this.compareMain(A) || this.comparePre(A);
    }
    compareMain(A) {
      if (!(A instanceof Sy)) A = new Sy(A, this.options);
      return bXA(this.major, A.major) || bXA(this.minor, A.minor) || bXA(this.patch, A.patch);
    }
    comparePre(A) {
      if (!(A instanceof Sy)) A = new Sy(A, this.options);
      ...
      do {
        let q = this.prerelease[K],
          Y = A.prerelease[K];
        if (i51("prerelease compare", K, q, Y), q === void 0 && Y === void 0) return 0;else if (Y === void 0) return 1;else if (q === void 0) return -1;else if (q === Y) continue;else return bXA(q, Y);
      } while (++K);
    }
    compareBuild(A) {
      if (!(A instanceof Sy)) A = new Sy(A, this.options);
      ...
      do {
        let q = this.build[K],
          Y = A.build[K];
        if (i51("build compare", K, q, Y), q === void 0 && Y === void 0) return 0;else if (Y === void 0) return 1;else if (q === void 0) return -1;else if (q === Y) continue;else return bXA(q, Y);
      } while (++K);
    }
    inc(A, K, q) {
      if (A.startsWith("pre")) {
        ...
        if (K) {
          let Y = `-${K}`.match(this.options.loose ? r51[o51.PRERELEASELOOSE] : r51[o51.PRERELEASE]);
