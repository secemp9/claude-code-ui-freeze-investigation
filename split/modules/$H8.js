...
var $H8 = v((Vkz, XH8) => {
  ...
  function NMq(A) {
    let K = A.COMMENT("//", "$", {
      ...
      Y = "[a-zA-Z_]\\w*::",
      z = "<[^<>]+>",
      w = "(decltype\\(auto\\)|" + MaA("[a-zA-Z_]\\w*::") + "[a-zA-Z_]\\w*" + MaA("<[^<>]+>") + ")",
      H = {
        className: "keyword",
        begin: "\\b[a-z\\d_]*_t\\b"
      },
      ...
      _ = {
        className: "title",
        begin: MaA("[a-zA-Z_]\\w*::") + A.IDENT_RE,
        relevance: 0
      },
      G = MaA("[a-zA-Z_]\\w*::") + A.IDENT_RE + "\\s*\\(",
      W = {
      ...
      D = {
        ...
        relevance: 0,
        keywords: W,
        begin: Xx1(/\b/, /(?!decltype)/, /(?!if)/, /(?!for)/, /(?!while)/, A.IDENT_RE, fMq(/\s*\(/))
      },
